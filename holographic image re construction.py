...
------------------------------------------------------------
Uses Masked Inverse-FFT to instantaneously sum tens of thousands 
of Moiré lattices without floating-point degradation.

Drag the slider to watch the world materialize from pure wave interference.
"""
...
class MoireWorldMakerV2(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Deterministic World Maker v2 (True Wave Interference)")
        ...
        self.max_lattices = self.img_size * self.img_size  # Total possible waves (65,536)
        self.current_lattices = 100
    ...
    def init_ui(self):
        ...
        self.lbl_slider = QLabel(f"Active Lattices (Koopman Modes): {self.current_lattices}")
        ...
        vbox2 = QVBoxLayout()
        vbox2.addWidget(QLabel("2. Active Genomic Space"))
        self.lbl_fft = QLabel()
    ...
    def update_reconstruction(self):
        ...
        if self.current_lattices >= self.max_lattices:
            threshold = -1 # Allow everything
        else:
            threshold = self.sorted_magnitudes[self.current_lattices - 1]
